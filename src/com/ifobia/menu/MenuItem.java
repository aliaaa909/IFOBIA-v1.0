...
public class MenuItem extends JPanel {
    ...
    private Icon getIcon() {
        ...
        // Jika menu item memiliki icon name (index 1), gunakan itu, jika tidak gunakan by index
        String iconName;
        if (menus.length > 1 && menus[1] != null && !menus[1].isEmpty()) {
            iconName = menus[1];  // Ambil dari array (misal: logout.svg)
        } else {
            iconName = menuIndex + ".svg";  // Gunakan by index
        }
        
        FlatSVGIcon icon = new FlatSVGIcon("com/ifobia/icon/" + iconName);
        ...
        for (int i = 0; i < menus.length; i++) {
            ...
            if (i == 0) {
                ...
                menuItem.addActionListener((ActionEvent e) -> {
                    // Jika menu KELUAR (hanya 1 element), langsung trigger logout
                    if (menus.length == 1 || menus[0].equals("KELUAR")) {
    ...
    protected void setSelectedIndex(int index) {
        ...
        // Deselect dan hilangkan focus dari semua button
    ...
    public void paint(Graphics g) {
        super.paint(g);
        // Jangan tampilkan panah untuk menu KELUAR
        if (menus.length > 1 && !menus[0].equals("KELUAR")) {
            Graphics2D g2 = (Graphics2D) g.create();
            g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            g2.setRenderingHint(RenderingHints.KEY_STROKE_CONTROL, RenderingHints.VALUE_STROKE_PURE);
